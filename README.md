...
- **リスト (list)**: 箇条書き項目（•付き）
- **水平線 (hr)**: 区切り線（編集不可）
- **見出し (heading)**: 太字見出し。警告マーク（⚠）をホバーするとGTD処理手順を表示
...
#### GTD処理手順機能
- 設定画面で「GTD処理手順」を設定可能（複数行テキスト）
- 見出しの右側に警告マーク（⚠）が表示される
- 警告マークにマウスホバーすると、設定したGTD処理手順がツールチップで表示される
- GTD処理手順が未設定の場合は、ツールチップは表示されない
...
- **保存**: SQLite3 経由の API
- **データ構造**: `{ id, type, text, checked, order, decoration, deadline }`
  - `type`: 'text' | 'checkbox' | 'list' | 'hr' | 'heading'
  - `order`: 入力順（ソートキー）
  - `decoration`: 装飾プリセット情報
  - `deadline`: 納期（ISO形式の日付文字列）
