...
### 初回ログイン
- ユーザーIDとパスワードを入力して新規登録
- ユーザーIDは英数字とハイフンのみ使用可能

### ユーザー名・パスワード記憶機能
- ログイン時に「ユーザー名・パスワードを記憶する」をチェックすると、次回以降自動でログイン情報が復元されます
- 記憶情報はブラウザのクッキーに安全なトークンとして保存されます（30日間有効）
- ログアウト時に記憶情報は自動的に削除されます
- **注意**: 共用端末や公開端末では、この機能を使用しないでください

### セキュリティ
- パスワードはハッシュ化されてデータベースに保存されます
- 記憶機能はパスワードそのものではなく、安全なトークンを使用します
- トークンは30日で自動的に期限切れになります
- ログアウト時にすべてのトークンが削除されます
...
3. ログイン:
   - 初回アクセス時はユーザーIDとパスワードを設定
   - 次回以降は「記憶する」にチェックを入れると自動ログイン可能
